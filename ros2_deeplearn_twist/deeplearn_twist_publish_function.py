...
DEEPLEARN = os.path.join(get_package_share_directory('ros2_deeplearn_twist'), 
                                                                 "deeplearn")

sys.path.insert(1, DEEPLEARN)
from _train_test import NNTools


#___Global Variables:
DEEP_SETTINGS = os.path.join(DEEPLEARN, 'settings.json')
...
def angular_calibration(servo, motor, ang_z, lin_x):
    ...
    This function proper angular value for robot movement.
...
def linear_calibration(servo, motor, ang_z, lin_x):
    ...
    This function proper linear value for robot movement.
...
def main(args=None):
    ...
    servo_pred = NNTools(DEEP_SETTINGS, ['servo','test'], 
                         os.path.join(DEEPLEARN, 'servo.pth'))
    motor_pred = NNTools(DEEP_SETTINGS, ['motor','test'], 
                         os.path.join(DEEPLEARN, 'motor.pth'))
